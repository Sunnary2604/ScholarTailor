...
class DataService:
    ...
    def __init__(self, db_manager=None, scholars_dir=None, db_path=None, output_file=None):
        """初始化服务类
        ...
        try:
            ...
            for scholar_id in main_scholar_ids:
                try:
                    ...
                    self.logger.info(f"学者 {scholar_id} 有 {len(relationships)} 个协作者")
            ...
            # 日志输出关联学者连接情况
            self.logger.info(f"找到 {len(secondary_scholar_connections)} 个关联学者，关系总数: {len(all_edges)}")
            ...
            self.logger.info(f"总节点数: {total_nodes_count}, 是否显示所有关联学者: {show_all_secondary}")
            print(f"总节点数: {total_nodes_count}, 是否显示所有关联学者: {show_all_secondary}")
            # 选择关联学者
            significant_secondary_scholars = {}
            if show_all_secondary:
                # 显示所有关联学者
                significant_secondary_scholars = secondary_scholar_connections
                self.logger.info(f"显示所有 {len(significant_secondary_scholars)} 个关联学者")
            else:
                # 检查是否有连接数大于2的关联学者
                ...
                if not high_connection_scholars:
                    self.logger.info("没有连接数大于2的关联学者，降低筛选标准，显示所有关联学者")
                    significant_secondary_scholars = secondary_scholar_connections
                else:
                    significant_secondary_scholars = high_connection_scholars
                    self.logger.info(f"只显示连接数大于2的关联学者，筛选后剩余 {len(significant_secondary_scholars)} 个")
            ...
            for scholar_id in significant_secondary_scholars.keys():
                try:
                    ...
                    if not entity or 'name' not in entity:
                        self.logger.warning(f"未找到关联学者 {scholar_id} 的实体信息或缺少name字段")
                        continue
                    ...
                    if entity.get('data'):
                        try:
                            ...
                            self.logger.warning(f"解析关联学者ID {scholar_id} 的额外数据失败: {str(e)}")
                        except Exception as e:
                            self.logger.warning(f"处理关联学者ID {scholar_id} 的额外数据时出错: {str(e)}")
                    
                    nodes.append(node)
                    self.logger.info(f"添加关联学者节点: {scholar_id}, {entity['name']}")
            ...
            self.logger.info(f"筛选前共 {len(all_edges)} 条边，筛选后共 {len(edges)} 条边")
            ...
            self.logger.info(f"生成网络数据：{len(nodes)}个节点，{len(edges)}条边 (主要学者:{len(main_scholar_ids)}，关联学者:{len(significant_secondary_scholars)})")
    ...
    def filter_network_data(self, filter_params):
        ...
        try:
            ...
            if not isinstance(sql_parts, dict):
                self.logger.error(f"_build_filter_sql返回的sql_parts不是字典类型: {type(sql_parts)}")
            ...
            # 获取符合筛选条件的学者ID列表
            try:
                scholar_ids = self._execute_filter_query(sql_parts)
            except Exception as e:
                self.logger.error(f"执行查询时出错: {str(e)}")
                return {
                    'success': False,
                    'error': f'执行筛选查询时出错: {str(e)}'
                }
            ...
            self.logger.info(f"筛选找到 {len(scholar_ids)} 个符合条件的学者，生成网络数据")
    ...
    def _build_filter_sql(self, filter_params):
        ...
        try:
            # 验证filter_params类型
            if not isinstance(filter_params, dict):
                self.logger.error(f"_build_filter_sql接收到非字典类型参数: {type(filter_params)}")
                # 返回默认查询字典
                return {
                    'select': 'SELECT DISTINCT s.scholar_id',
                    'from': 'FROM scholars s',
            ...
            if filter_params.get('interestKeyword'):
                sql_parts['joins'].append('LEFT JOIN interests i ON i.entity_id = s.scholar_id')
            ...
            if filter_params.get('tagFilter'):
                ...
                try:
                    ...
                    if tag_tables:
                        ...
                        sql_parts['joins'].append(f'LEFT JOIN {tag_table} t ON t.entity_id = s.scholar_id')
                        sql_parts['where'].append('t.tag = ?')
                        sql_parts['params'].append(filter_params['tagFilter'])
                    else:
                        # 如果没有找到tag相关的表，尝试使用interests表
                        self.logger.warning("未找到标签表，尝试使用interests表进行标签筛选")
                        sql_parts['joins'].append('LEFT JOIN interests i ON i.entity_id = s.scholar_id')
                        sql_parts['where'].append('i.interest = ?')
                        sql_parts['params'].append(filter_params['tagFilter'])
                except Exception as e:
            ...
            if filter_params.get('affiliationKeywordStartsWith'):
                sql_parts['where'].append('s.affiliation LIKE ?')
                sql_parts['params'].append(f'{filter_params["affiliationKeywordStartsWith"]}%')
            ...
            # 如果有论文相关筛选条件，添加JOIN
            if has_pub_filter:
                sql_parts['joins'].append('LEFT JOIN authorship a ON a.scholar_id = s.scholar_id')
                sql_parts['joins'].append('LEFT JOIN publications p ON p.cites_id = a.cites_id')
            ...
            # 如果有机构相关筛选条件，添加JOIN
            if has_inst_filter:
                sql_parts['joins'].append('LEFT JOIN scholar_institutions si ON si.scholar_id = s.scholar_id')
                sql_parts['joins'].append('LEFT JOIN institutions i ON i.inst_id = si.inst_id')
    ...
    def _execute_filter_query(self, sql_parts):
        """执行筛选查询并返回结果
    ...
    def _generate_filtered_network_data(self, scholar_ids, filter_params):
        ...
        for scholar_id in scholar_ids:
            try:
                ...
                entity = self.entity_dao.get_entity_by_id(scholar_id)
                if not entity or 'name' not in entity:
                    self.logger.warning(f"未找到学者ID {scholar_id} 的实体信息或缺少name字段")
                    continue
                ...
                try:
                    ...
                    self.logger.warning(f"获取学者ID {scholar_id} 的兴趣标签时出错: {str(e)}")
                    interests = []
                ...
                node = {
                    ...
                    'group': 'primary' if scholar.get('is_main_scholar') else 'secondary',
                    'data': {
                        'id': scholar_id,
                ...
                if entity.get('data'):
                    try:
                        ...
                        self.logger.warning(f"处理学者ID {scholar_id} 的额外数据时出错: {str(e)}")
        ...
        for source_id in valid_scholar_ids:
            try:
                # 获取所有与该学者相关的关系
                relationships = self.relationship_dao.get_collaborators(source_id)
                ...
                for rel in relationships:
                    try:
                        ...
                        if target_id in valid_scholar_ids and rel.get('relation_type', 'coauthor') in relation_filters:
                            # 创建唯一边标识符
                        ...
                        self.logger.warning(f"处理学者 {source_id} 的关系 {rel} 时出错: {str(e)}")
                        continue
            except Exception as e:
        ...
        try:
            ...
            for file_name in json_files:
                try:
                    ...
                    try:
                        # 导入学者数据
                        import_result = scholar_service._import_scholar_complete(scholar_id, scholar_data)
                        ...
                        if import_result['success']:
                            ...
                            # 回滚事务
                            db_manager.rollback()
                            self.logger.error(f"导入学者失败: {file_name}, 错误: {import_result.get('error')}")
                    
                    except Exception as inner_error:
                        # 回滚事务
                        db_manager.rollback()
                        self.logger.error(f"导入学者时出错: {file_name}, 错误: {str(inner_error)}")
                
                except Exception as file_error:
                    self.logger.error(f"处理文件时出错: {file_name}, 错误: {str(file_error)}")
