...
function cacheScholars(data) {
        ...
        if (node.data && node.group === 'primary') {
            ...
            // 从custom_fields.tags中恢复标签数据
                ...
                if (tagStr && typeof tagStr === 'string') {
                    node.data.tags = tagStr.split(',').map(tag => tag.trim()).filter(tag => tag);
                }
            }
    ...
    // 找出并缓存自定义关系
    if (data.edges) {
        customRelationships = data.edges.filter(edge => 
            edge.label !== 'coauthor' || 
            edge.data && edge.data.is_custom
        );
    }
    
    const primaryCount = Object.values(scholars).filter(s => !s.is_secondary).length;
    const secondaryCount = Object.values(scholars).filter(s => s.is_secondary).length;
    
    console.log(`已缓存 ${Object.keys(scholars).length} 位学者数据 (主要: ${primaryCount}, 潜在: ${secondaryCount})`);
}
...
function initGraph(data) {
    ...
    cy = cytoscape({
        ...
        layout: { 
            ...
            padding: 50,
            nodeOverlap: 20,
            idealEdgeLength: 150,  // 增加理想边长度，让节点间距更大
            nodeRepulsion: 8000,   // 增加节点斥力，降低密度
            nestingFactor: 0.2     // 减少节点嵌套
        },
        minZoom: 0.2,
        maxZoom: 3,
    ...
    graphControls.innerHTML = `
        ...
        <button id="graph-view-btn" class="control-btn" title="返回全局视图">
            <i class="fas fa-globe"></i> 返回全局视图
        </button>
    `;
    document.getElementById('cy').appendChild(graphControls);
    ...
    if (data.edges) {
        ...
        for (const edge of data.edges) {
            ...
            if (existingEdges[edgeId]) {
                // 合并标签，如"合作者,导师"
                const existingEdge = existingEdges[edgeId];
                if (!existingEdge.data.label.includes(edge.label)) {
                    existingEdge.data.label += `, ${edge.label}`;
                    existingEdge.data.multiRelation = true;
                    
                    // 标记多重关系的样式
                    existingEdge.data.relationTypes = existingEdge.data.relationTypes || [existingEdge.data.originalType];
                    existingEdge.data.relationTypes.push(edge.label);
                }
                
                // 增加权重
                existingEdge.data.weight += edge.weight || 1;
            } 
            // 如果存在相反方向的边
            else if (existingEdges[reverseEdgeId]) {
                // 合并标签，如"合作者,导师"
                const existingEdge = existingEdges[reverseEdgeId];
                if (!existingEdge.data.label.includes(edge.label)) {
                    existingEdge.data.label += `, ${edge.label}`;
                    existingEdge.data.multiRelation = true;
...
function setupEventListeners() {
    ...
    // 关系过滤器
    const relationCheckboxes = document.querySelectorAll('.relation-filters input[type="checkbox"]');
    ...
    document.body.addEventListener('click', function(event) {
        if (event.target.id === 'graph-view-btn' || event.target.closest('#graph-view-btn')) {
            resetToGlobalView();
        }
    });
...
function setupAdminPanelEvents() {
    ...
    // 添加学者按钮
    document.getElementById('add-scholar-btn').addEventListener('click', addNewScholar);
    
    // 批量添加学者按钮
    document.getElementById('batch-add-btn').addEventListener('click', batchAddScholars);
    ...
    // 添加关系按钮
    document.getElementById('add-relation-btn').addEventListener('click', addCustomRelationship);
}
...
function loadAdminPanelData() {
    ...
    document.getElementById('last-update-time').textContent = formatDateTime(lastUpdateTime);
...
function loadRelationshipList() {
    ...
    if (customRels.length === 0) {
        relationshipListEl.innerHTML = '<div class="empty-list">暂无自定义关系</div>';
        return;
    }
    ...
    relationshipListEl.innerHTML = listHTML || '<div class="empty-list">暂无主要学者之间的关系</div>';
    ...
    deleteBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            // 确认删除
            if (confirm(`确定要删除 ${scholars[sourceId]?.name || sourceId} 和 ${scholars[targetId]?.name || targetId} 之间的${relType === 'advisor' ? '导师' : relType === 'colleague' ? '同事' : '合作者'}关系吗？`)) {
                item.remove();
...
function changeLayout(name) {
    ...
    cy.layout(layoutConfig[name] || layoutConfig.cose).run();
}
...
function searchScholar() {
    ...
    const matchedNodes = cy.nodes().filter(node => {
        ...
        return name.includes(searchTerm) || 
               ...
               levenshteinDistance(name, searchTerm) <= 2; // 编辑距离不超过2
    });
...
function enableFocusMode(centerNode) {
    ...
    cy.elements().difference(relevantElements).addClass('hidden');
...
function resetToGlobalView() {
    ...
    // 显示所有节点
    cy.elements().removeClass('hidden highlighted related faded');
...
function updateDetailPanel(scholarData) {
    ...
    if (isSecondary) {
        ...
        if (scholarData.interests && scholarData.interests.length > 0) {
            ...
            scholarData.interests.forEach(interest => {
                interestsEl.innerHTML += `<span class="interest-tag">${interest}</span>`;
            });
        } else {
    ...
    citationsEl.textContent = scholarData.citedby ? scholarData.citedby.toLocaleString() : '未知';
    ...
    if (isSecondary) {
        ...
        // 爬取按钮
        buttonsHtml += `
            <button id="fetch-scholar-btn" class="scholar-button">
        ...
        if (scholarData.scholar_url) {
            buttonsHtml += `
                <a href="${scholarData.scholar_url}" target="_blank" class="scholar-button">
        ...
        buttonsHtml += '</div>';
        customFieldsEl.innerHTML += buttonsHtml;
        ...
        customFieldsEl.innerHTML = '<p>无额外信息</p>';
    ...
    if (isSecondary) {
        ...
        for (const pub of scholarData.publications) {
            ...
            if (pub.venue || pub.year) {
                venueInfo += '<div class="pub-meta">';
                if (pub.venue) {
                    venueInfo += `<span class="venue-badge">${pub.venue}</span>`;
                }
                if (pub.year) {
                    venueInfo += `<span class="pub-year">${pub.year}</span>`;
                }
                if (pub.citations) {
                    venueInfo += `<span class="pub-citations"><i class="fas fa-quote-right"></i> ${pub.citations}</span>`;
                }
                venueInfo += '</div>';
            }
        ...
        publicationListEl.innerHTML = '<li class="no-pub">无论文信息</li>';
    }
...
function updateRelatedScholars(scholarId) {
    ...
    connectedEdges.forEach(edge => {
        ...
        if (relatedNode.length > 0) {
            ...
            const relationTypes = edge.data('relationTypes') || [edge.data('label')];
            ...
            relationTypes.forEach(type => {
                ...
                relationBadges += `<span class="relation-badge ${type}">${relationTypeChinese}</span>`;
            });
            
            relatedHTML += `<li data-id="${relatedId}" class="${isSecondary ? 'secondary-scholar' : 'primary-scholar'}">
                <div class="relation-name">${relatedData.name}</div>
                <div class="relation-type">
                    ${relationBadges}
                    ${edge.data('weight') > 1 ? `<span class="relation-weight">权重: ${edge.data('weight')}</span>` : ''}
                    ${isSecondary ? '<span class="secondary-badge">潜在</span>' : ''}
                </div>`;
                
            // 添加研究方向标签
            if (relatedData.interests && relatedData.interests.length > 0 && !isSecondary) {
                ...
                if (relatedData.interests.length > 3) {
                    relatedHTML += `<span class="interest-more">+${relatedData.interests.length - 3}</span>`;
                }
                
                relatedHTML += '</div>';
            }
            
            relatedHTML += `<div class="relation-affiliation">${relatedData.affiliation || ''}</div>
            </li>`;
        }
    });
    ...
    // 添加点击事件跳转到相关学者
    const relatedItems = relatedScholarsEl.querySelectorAll('li[data-id]');
...
function clearDetailPanel() {
    ...
    document.getElementById('publication-list').innerHTML = '<li>选择学者查看论文</li>';
    document.getElementById('related-scholars').innerHTML = '<li>选择学者查看关系</li>';
}
...
async function addNewScholar() {
    ...
    if (!scholarName && !scholarId) {
        showAdminStatus('请输入学者名称或Scholar ID', 'error');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            showAdminStatus(`成功添加学者 "${result.message.replace('成功添加学者 ', '')}"`, 'success');
            scholarInput.value = '';
...
async function batchAddScholars() {
    ...
    const scholarNames = batchTextarea.value.trim().split('\n')
        .map(name => name.trim())
    ...
    showAdminStatus(`正在批量爬取 ${scholarNames.length} 位学者的数据...`, 'info');
    ...
    try {
        ...
        if (result.success) {
            ...
            showAdminStatus(`批量添加学者失败: ${result.error || '未知错误'}`, 'error');
        }
    } catch (error) {
...
async function updateScholar(scholarId) {
    ...
    if (!confirm(`确定要更新 "${scholarName}" 的数据吗？这将从Google Scholar重新爬取数据。`)) {
        return;
    }
...
function editCustomFields(scholarId) {
    ...
    modal.querySelector('#add-field-btn').addEventListener('click', () => {
        ...
        fieldContainer.insertBefore(newField, modal.querySelector('#add-field-btn'));
    ...
    // 保存字段
    modal.querySelector('#save-fields-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                showAdminStatus(`更新自定义字段失败: ${result.error || '未知错误'}`, 'error');
            }
        } catch (error) {
...
async function refreshAllData() {
    ...
    if (!confirm(`确定要更新所有学者数据吗？这将从Google Scholar重新爬取所有学者的数据，${keepCustom ? '并保留' : '但不保留'}自定义关系。`)) {
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            showAdminStatus(`成功更新 ${result.updated || 0} 位学者的数据`, 'success');
            
            // 重新加载数据
            await reloadData();
        } else {
            showAdminStatus(`更新所有数据失败: ${result.error || '未知错误'}`, 'error');
        }
    } catch (error) {
...
async function addCustomRelationship() {
    ...
    // 确认添加关系
    if (!confirm(`确定要添加 ${sourceName} 作为 ${targetName} 的${relationTypeChinese}关系吗？`)) {
        return;
    }
...
async function fetchSecondaryScholarData(scholarId, scholarName) {
    ...
    try {
        ...
        if (fetchBtn) {
            ...
            fetchBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 正在爬取数据...';
        }
...
function setupTagFiltering() {
    ...
    if (allTags.size > 0) {
        ...
        tagFiltersContainer.innerHTML = '<label><strong>按标签筛选:</strong></label>';
        ...
        allTags.forEach(tag => {
            ...
            tagSpan.className = 'scholar-tag filter-tag';
        ...
        document.querySelector('.graph-controls').appendChild(tagFiltersContainer);
    }
}
...
async function initializeDatabase() {
    if (!confirm('警告：此操作将清空所有数据，包括学者数据、自定义关系和标签。此操作不可恢复！确定要继续吗？')) {
        return;
    }
