...
export function initGraph(containerId, data, perfOptions = {}) {
  ...
  if (containerWidth < 10 || containerHeight < 10) {
    console.warn(`警告: 图谱容器尺寸过小 (${containerWidth}x${containerHeight})`);
    // 强制设置最小尺寸
...
export function getFCoseLayoutOptions() {
  return {
    name: 'fcose',
    // 使用高质量布局，但考虑性能
    quality: 'default', // 使用default而非proof以提高性能
    // 随机初始位置
    randomize: true,
    // 理想边长 - 可以是数值或函数
    idealEdgeLength: 300, // 使用固定值，避免报错
    // 节点间隔
    nodeOverlap: 120, // 增加节点间隔
    // 节点斥力 - 可以是数值或函数
    nodeRepulsion: 25000, // 增加为固定值，避免报错
    // 考虑节点标签
    nodeDimensionsIncludeLabels: true,
    // 弹性系数 - 可以是数值或函数
    edgeElasticity: 0.4, // 使用固定值
    // 布局扩散因子
    nestingFactor: 2, // 增加扩散因子
    // 重力常数 - 较小的值让节点可以更分散
    gravity: 0.2, // 减小重力常数
    // 最大迭代次数
    numIter: 3000, // 平衡性能和布局质量
    // 允许和平坦处理
    tile: true,
    // 平铺间隔 - 使用固定值
    tilingPaddingVertical: 40, // 增加平铺间隔
    tilingPaddingHorizontal: 40, // 增加平铺间隔
    // 布局适配画布
    fit: true,
    // 画布边距
    padding: 70, // 增加画布边距
    // 动画
    animate: true,
    animationDuration: 3500, // 增加动画持续时间
    animationEasing: 'ease-in-out', // 修改动画缓动函数
    // 性能优化
    refresh: 10, // 每10次迭代刷新一次视图
    packComponents: true, // 优化组件打包
    // 初始冷却因子，较低的值可以产生更好的布局
...
export function adjustNodeSizeByConnections(cy) {
  ...
  cyToUse.nodes().forEach(node => {
    ...
    let maxSize = nodeType === 'primary' ? 60 : 36;
    
    // 连接数权重因子 (0-1)
    const connectionFactor = maxConnections > 1 ? connections / maxConnections : 0;
